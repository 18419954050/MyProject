...
class WechatAuth {
    private $cacheTokenFileName = "./api/cache_token.json";
    /* 消息类型常量 */
     ...
     */
    public function getAccessToken($type , $code = null){
        $param = array(
            'appid'  => $this->appId,
        ...
        switch ($type) {
            case 'client':
                $data = json_decode(@file_get_contents($this->cacheTokenFileName),true);
                if ($data["expire_time"] < time()) {
                    ...
                    if ($token) {
                        ...
                        $fp = fopen($this->cacheTokenFileName, "w");
    ...
    public function getToken(){
        ...
        $data = json_decode(file_get_contents($this->cacheTicketFileName),true);
        if ($data['expire_time'] < time()) {
            $token = self::http($url, $param);
            $token = json_decode($token, true);
            ...
            if ($access_token2) {
                ...
                $fp = fopen($this->cacheTicketFileName, "w");
                fwrite($fp, json_encode($data));
                fclose($fp);
            }
            return $token;
    ...
    public function getInfo($token,$token2, $lang = 'zh_CN'){
        $query = array(
    ...
    public function getIndustry($token){
        ...
        $industry = self::http("{$this->getIndustryURL}", $query);
        return json_decode($industry, true);
    }

    public function getAllPrivateTemplate($token){
        $query = array(
            'access_token' => $token['access_token'],
        );
        $Template = self::http("{$this->getAllPrivateTemplateURL}", $query);
        return json_decode($Template, true);
    }

    public function _sendIndustry($name, $data = '', $method = 'POST', $param = ''){
        $params = array('access_token' => $this->accessToken);
